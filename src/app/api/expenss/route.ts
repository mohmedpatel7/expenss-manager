...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    // Parse request body
    const { categoryId, amount: rawAmount } = await req.json();
    const amount = Number(rawAmount);
    if (isNaN(amount) || !categoryId || amount <= 0) {
      return NextResponse.json(
        { message: "Missing or invalid fields" },
        { status: 400 }
      );
    ...
    // Find the expense category by _id and userId
    const expenseCategory = await Expenss.findOne({ _id: categoryId, userId });
    if (!expenseCategory) {
      return NextResponse.json(
    ...
    // Send email notification (reuse logic from POST)
    await transporter.sendMail({
      from: process.env.EMAIL,
      to: email,
      subject: "Expense Recorded - Credit Deducted",
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 8px;">
          <h2 style="color: #333; text-align: center; margin-bottom: 30px;">Expense Recorded</h2>
          <div style="background-color: #ffe8e8; padding: 20px; border-radius: 6px; margin-bottom: 20px;">
            <h3 style="color: #5a2d2d; margin: 0 0 10px 0;">💸 Expense Deducted</h3>
            <p style="font-size: 18px; font-weight: bold; color: #5a2d2d; margin: 0;">Amount: ₹${amount.toLocaleString()}</p>
            <p style="font-size: 16px; color: #5a2d2d; margin: 10px 0 0 0;">Category: ${
              expenseCategory.title.charAt(0).toUpperCase() +
              expenseCategory.title.slice(1)
            }</p>
          </div>
          <div style="background-color: #f8f9fa; padding: 15px; border-radius: 6px; margin-bottom: 20px;">
            <h4 style="color: #333; margin: 0 0 10px 0;">Account Summary</h4>
            <p style="margin: 5px 0; color: #666;"><strong>Previous Balance:</strong> ₹${(
              creditAccount.currentCredit + amount
            ).toLocaleString()}</p>
            <p style="margin: 5px 0; color: #666;"><strong>Current Balance:</strong> ₹${creditAccount.currentCredit.toLocaleString()}</p>
            <p style="margin: 5px 0; color: #666;"><strong>Transaction Type:</strong> Expense (Debit)</p>
          </div>
          <div style="text-align: center; color: #666; font-size: 14px;">
            <p>This is an automated notification from your Expense Management System.</p>
            <p>If you didn't perform this transaction, please contact support immediately.</p>
